...
export class JobSchedulerService {
    ...
    private async ensureQueueExists(queueName: string): Promise<void> {
        try {
            ...
            logger.info(`Ensured queue exists: ${queueName}`);
        } catch (error: any) {
    ...
    private async registerWorkersForExistingSchedules(): Promise<void> {
        try {
            logger.info('Fetching existing schedules to register workers...');
            ...
            for (const schedule of schedules) {
                ...
                if (schedule.name.startsWith(QueuePrefix.ScheduledAction)) {
                    ...
                    logger.info(`Registering worker on restart for queue: ${schedule.name}`);
    ...
    private async handleScheduledAction(jobs: Job<any>[]): Promise<void> {
        logger.info(`Handling scheduled action jobs...`);
        if (!Array.isArray(jobs) || jobs.length === 0) {
            logger.error('Received empty job array');
            return;
        }

        const job = jobs[0];
        const scheduleName = job.name;
        logger.info(`Received job ${job.id} from queue ${scheduleName}`);

        if (!job.data || typeof job.data !== 'object') {
            logger.error(`Job ${job.id} has invalid or missing data.`);
        ...
        if (!actionId) {
            logger.error(`Job ${job.id} data is missing actionId.`);
            await this.boss.fail(job.id, 'Missing actionId in job data');
            return;
        }

        logger.info(`Processing job ${job.id} for action ${actionId}`);
        ...
        try {
            ...
            if (!action || !action.enabled) {
                logger.warn(`Action ${actionId} not found or disabled for job ${job.id}. Marking job as failed.`);
    ...
    private convertCronToLocalTimezone(cronExpression: string, timezone?: string): string {
        ...
        if (parts.length !== 5) {
            logger.warn('Invalid cron expression format, using as-is:', cronExpression);
    ...
    public async scheduleJob(action: ActionDefinition): Promise<string> {
        ...
        try {
            ...
            logger.info(`Registered worker for queue ${scheduleName}`);

            await this.boss.schedule(scheduleName, localCronExpression, jobData);
            logger.info(`Scheduled job creation for ${scheduleName}`);
    ...
    public async cancelJob(scheduleName: string): Promise<void> {
        ...
        try {
            ...
            logger.info(`Unscheduled job creation for ${scheduleName}`);
    ...
    public async rescheduleJob(action: ActionDefinition, oldScheduleName?: string): Promise<string> {
        ...
        if (oldScheduleName) {
            logger.info(`Rescheduling: Cancelling old schedule ${oldScheduleName}`);
        ...
        logger.info(`Rescheduling: Scheduling new job for action ${action.id}`);
    ...
    public async stop(): Promise<void> {
        if (this.boss && (await this.boss.isInstalled())) {
            logger.info('Stopping Job Scheduler (PgBoss)...');
            await this.boss.stop({graceful: true}); // Use graceful stop if available
            logger.info('Job scheduler stopped');
        } else {
            logger.info('Job scheduler is not running or already stopped');
        }
    }
}
